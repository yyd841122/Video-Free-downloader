...
- Douyin CDN 节点偶尔会读超时，后端下载会自动重试 4 次。
- 重试时会重新解析分享页刷新媒体地址，并使用 `Range` 从已下载位置续写。
- 如果服务端不支持 `Range`，后端会自动重新写入文件，避免拼出损坏文件。
...
## 14. Bilibili 高清免扫码方案调研

调研日期：2026-05-17。

目标问题：能否做到“云端网站下载 Bilibili 1080P+，同时不让用户扫码、不让用户提供 Cookie、不使用公共账号”。

结论：目前没有找到可行且安全的纯服务端方案。Bilibili 高清播放地址的返回仍依赖用户登录态、账号权限、会员权限和平台风控。Referer、Origin、User-Agent 只能提高兼容性，不能把无权限接口变成高清接口。

### 调研仓库

- `yutto-dev/yutto`
  - 活跃维护，2026-05-16 仍有提交。
  - README 中展示 1080P 高码率示例时，日志明确显示“大会员 成功以大会员身份登录”。
  - 近期功能包含 `yutto login` 保存认证信息。
  - 结论：高清能力来自本机配置的认证信息，不是免登录绕过。

- `nICEnnnnnnnLee/BilibiliDown`
  - 活跃维护，2026-04-24 有 V6.41 更新。
  - README 明确说明登录凭证明文保存在 `config/cookies.config`，并提供“登录相关/退出登录”等能力。
  - 项目还包含 Cookie 刷新逻辑，但前提仍是已有登录凭证。
  - 结论：可参考 Cookie 刷新和本地客户端体验，但不能解决纯云端免登录高清。

- `lanyeeee/bilibili-video-downloader`
  - Tauri 桌面应用，2026-03-18 发布 v0.2.0。
  - 功能表明确包含“Cookie 登录、二维码登录”，并支持 8K、杜比视界、HDR、4K 等画质。
  - 结论：高清能力依赖桌面端本地登录态，方向与“本地助手/桌面版”一致。

- `kangpeiqin/bilivideo_down`
  - Flutter 桌面应用，最近提交为 2025-02-07。
  - 仓库定位是 Windows/macOS 桌面端 Bilibili 下载器，未发现纯服务端免登录高清方案。
  - 结论：可作为桌面端形态参考，但不是当前网站模式的高清突破口。

### 可落地路线

1. 继续保留当前网站模式：
   - 无登录：支持 Bilibili 公开可返回的清晰度，通常到 480P 或部分视频 720P。
   - 用户扫码：后端临时内存保存会话 Cookie，用于解析 1080P+。
   - 风险：云端接触用户登录态，不适合面向公众长期开放。

2. 新增 `SaveAny Local Helper` 本地助手：
   - 网站仍负责 UI、任务编排、会员/付费、通用平台下载体验。
   - 用户本机运行一个 localhost 小程序，由它读取本机浏览器 Cookie 或完成本机二维码登录。
   - Bilibili 高清解析和下载在用户本机完成，Cookie 不上传到云端。
   - 网站通过 `http://127.0.0.1:<port>` 与本地助手通信，只拿任务状态和最终文件。
   - 这是兼顾易用性、安全性和高清能力的推荐路线。

3. 封装桌面版：
   - 直接把 Vue + FastAPI 或核心下载服务封装成桌面应用。
   - 适合重度用户，但会偏离“纯网站随时随地使用”的产品定位。

不推荐路线：

- 公共账号池：账号封禁、责任归属和合规风险都高。
- 云端静默收集用户 Cookie：安全风险高，用户信任成本极高。
- 宣称无登录稳定支持 1080P+：与当前 Bilibili 权限模型不一致，容易形成产品承诺风险。
